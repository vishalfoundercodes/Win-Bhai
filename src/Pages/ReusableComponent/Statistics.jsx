...
//   const [filter, setFilter] = useState("all"); // all | today | period
...
//     setSelecting(null);
//     setOpenModal(null); // ✅ close ONLY here
//   }
// };



//   const periodData = stats?.daily_breakdown?.find(
//     (d) => d.date === selectedDate
//   );
//   const periodRangeData =
//     startDate && endDate
...
//                   }
//                 }}
//                 className="px-3 py-1 rounded-md border border-gray-300 flex-1 bg-white focus:outline-none cursor-pointer"
//               >
...
//                     </td>
//                     <td className="py-2 px-2 border border-lightBorder">
//                       {todayData?.registrations || 0}
//                     </td>
//                     <td className="py-2 px-2 border border-lightBorder">
//                       {todayData?.first_deposits || 0}
//                     </td>
//                     <td className="py-2 px-2 border border-lightBorder">
//                       {todayData?.total_deposit || 0}
//                     </td>
//                     <td className="py-2 px-2 border border-lightBorder">
//                       {todayData?.total_withdrawal || 0}
//                     </td>
//                     <td className="py-2 px-2 border border-lightBorder">
//                       {todayData?.total_loss || 0}
//                     </td>
//                     <td className="py-2 px-2 border border-lightBorder">
//                       {todayData?.total_commission || 0}
//                     </td>
//                   </tr>
//                 )}

//                 {/* PERIOD VIEW (placeholder) */}
//                 {/* PERIOD VIEW */}
//                 {filter === "period" &&
//                   (startDate && endDate ? (
//                     periodRangeData.length ? (
//                       periodRangeData.map((day, idx) => (
//                         <tr key={idx}>
//                           <td className="py-2 px-2 border border-lightBorder text-start lg2:text-center">
//                             {/* {day.date} */}
...
//           onClose={() => {
//             // ⛔ Start select ho raha hai to close mat hone do
...
export default function Statistics() {
  ...
  const handleDateSelect = (date) => {
    ...
    if (selecting === "start") {
      setStartDate(formattedDate);
      toast.success(`✅ Start Date Selected: ${formattedDate}`, {
        position: "top-right",
        autoClose: 2000,
      });
      setSelecting("end");
    ...
    if (selecting === "end") {
      setEndDate(formattedDate);
      toast.success(`✅ End Date Selected: ${formattedDate}`, {
        position: "top-right",
        autoClose: 2000,
      });
      setSelecting(null);
  ...
  // Calculate totals for period range
  const calculatePeriodTotals = () => {
    if (!periodRangeData.length) return null;

    return periodRangeData.reduce(
      (acc, day) => ({
        registrations: acc.registrations + (day.registrations || 0),
        first_deposits: acc.first_deposits + (day.first_deposits || 0),
        total_deposit: acc.total_deposit + (day.total_deposit || 0),
        total_withdrawal: acc.total_withdrawal + (day.total_withdrawal || 0),
        total_loss: acc.total_loss + (day.total_loss || 0),
        total_commission: acc.total_commission + (day.total_commission || 0),
      }),
      {
        registrations: 0,
        first_deposits: 0,
  ...
  const todayData = stats?.daily_breakdown?.find(
        ...
        <div className="relative z-1 max-w-3xl lg2:max-w-full mx-auto px-4 lg2:px-0 py-6 lg2:py-0 lg2:pr-4">
          <div className="flex flex-col justify-between mb-6 gap-2 lg2:bg-white lg2:rounded-b-2xl lg2:py-6 lg2:px-4">
            ...
            <div className="flex items-center gap-2">
              ...
              {/* Period Date Display & Edit Button */}
              {filter === "period" && (
                <div className="flex items-center gap-2 flex-1">
                  <button
                    ...
                    className="px-3 py-1 bg-red text-white rounded-md text-sm hover:bg-red-700 transition"
                  >
                    {startDate || endDate ? "Change" : "Select"}
                  </button>
                </div>
              )}
              ...
              <button
                ...
                // onClick={()=>{const userId=localStorage.getItem("userId")}}
                onClick={() => {
                  const userId = localStorage.getItem("userId");
                  if (!userId) return;

                  const url = `${apis.statics_download}${userId}`;
                  window.open(url, "_self"); // new tab me hit hoga
                }}
              >
                <svg
                  width="16"
                ...
                {filter === "all" && (
                  <>
                    <tr className="bg-yellow-50">
                      <td className="py-2 px-2 border border-lightBorder text-start lg2:text-center font-semibold">
                        All Time
                      </td>
                      <td className="py-2 px-2 border border-lightBorder">
                        {stats?.all_time?.registrations || 0}
                      ...
                      periodRangeData.length ? (
                        <>
                          ...
                          <tr className="bg-green-50">
                            ...
                            <td className="py-2 px-2 border border-lightBorder font-semibold">
                              {periodTotals?.registrations || 0}
                            </td>
                            <td className="py-2 px-2 border border-lightBorder font-semibold">
                              {periodTotals?.first_deposits || 0}
                            </td>
                            <td className="py-2 px-2 border border-lightBorder font-semibold">
                              {periodTotals?.total_deposit || 0}
                            </td>
                            <td className="py-2 px-2 border border-lightBorder font-semibold">
                              {periodTotals?.total_withdrawal || 0}
                            </td>
                            <td className="py-2 px-2 border border-lightBorder font-semibold">
                              {periodTotals?.total_loss || 0}
                            </td>
                            <td className="py-2 px-2 border border-lightBorder font-semibold">
                              {periodTotals?.total_commission || 0}
                            </td>
                          </tr>
                          {/* Daily Breakdown */}
                          {periodRangeData.map((day, idx) => (
